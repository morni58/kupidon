/* ============================================================
   CupidBot — shared UI primitives
   ============================================================ */
const { useState, useEffect, useRef, useCallback } = React;

// ---------- Photo: gradient + big vibe emoji (stylised portrait) ----------
function Photo({ data, rounded = '2rem', className = '', emojiSize = 96, children, dim = false }) {
  if (!data) return null;
  return (
    <div
      ...
      style={{
        ...
        background: `linear-gradient(150deg, ${data.from}, ${data.to})`,
      }}
    >
      {/* soft light blobs for depth */}
      <div className="absolute -top-10 -left-8 w-40 h-40 rounded-full"
           style={{ background: 'rgba(255,255,255,0.35)', filter: 'blur(28px)' }} />
      <div className="absolute bottom-0 right-0 w-44 h-44 rounded-full"
           style={{ background: 'rgba(0,0,0,0.18)', filter: 'blur(34px)' }} />
      <div className="absolute inset-0 flex items-center justify-center select-none"
           style={{ fontSize: emojiSize, filter: 'drop-shadow(0 8px 16px rgba(0,0,0,0.18))' }}>
...
// ---------- Avatar (small round photo) ----------
function Avatar({ data, size = 48, ring = false, emoji }) {
  return (
...
// ---------- Button ----------
function Button({ variant = 'primary', children, onClick, disabled, className = '', style = {}, size = 'md' }) {
  ...
  const variants = {
    primary: 'text-white',
    secondary: 'bg-white text-[#0F0F13] border-2 border-[#e5e7eb]',
    ghost: 'text-[#6b7280] bg-transparent',
    gold: 'text-[#0F0F13]',
    blue: 'text-white',
  };
  const vstyle = {
...
// ---------- Toggle / switch ----------
function Toggle({ on, onChange, color = '#FF00FF' }) {
  return (
    <button
      onClick={() => onChange(!on)}
...
// ---------- Pill / tag ----------
function Pill({ interest, selected, onClick, dim, glass, small }) {
  ...
  if (glass) {
    return (
      <span className="inline-flex items-center gap-1 rounded-full font-semibold text-white"
        style={{ height: h, padding: '0 12px', fontSize: small ? 11 : 12.5,
  ...
  return (
    <button
      ...
      style={{
        height: h, padding: '0 14px', fontSize: 13,
        border: `2px solid ${i.color}`,
        background: selected ? i.color : 'transparent',
        color: selected ? (i.color === '#0F0F13' ? '#fff' : '#fff') : i.color === '#0F0F13' ? '#0F0F13' : i.color,
        opacity: dim ? 0.35 : 1,
        boxShadow: selected ? `0 6px 18px -6px ${i.color}` : 'none',
...
// ---------- Badge ----------
function Badge({ kind = 'verified', children, className = '' }) {
  const map = {
  ...
  return (
    <span className={'inline-flex items-center gap-1 rounded-full font-bold ' + className}
      style={{ background: m.bg, color: m.fg, fontSize: 11.5, padding: '4px 9px',
        border: kind === 'glass' ? '1px solid rgba(255,255,255,0.4)' : 'none',
        backdropFilter: kind === 'glass' ? 'blur(6px)' : 'none' }}>
      {children && kind !== 'common' ? children : m.txt}
    </span>
  );
}

function VerifiedTick({ size = 18 }) {
    ...
    <div className={'absolute inset-0 z-40 transition-all duration-300 ' + (open ? 'pointer-events-auto' : 'pointer-events-none')}>
      ...
      <div
        ...
        style={{
          ...
          padding: '12px 20px 28px', boxShadow: '0 -20px 50px rgba(0,0,0,0.25)',
        }}
      >
...
function Toast({ toast }) {
  ...
  return (
    <div className="absolute left-0 right-0 z-50 flex justify-center pointer-events-none"
      ...
      <div className="rounded-full font-semibold text-[13px] text-white shadow-lg animate-[toastIn_.3s_ease]"
           style={{ padding: '9px 16px', background: 'rgba(15,15,19,0.92)', backdropFilter: 'blur(8px)' }}>
        {toast}
      </div>
    </div>
  );
...
function TabBar({ active, onTab, accent = '#FF00FF', dark = false, dots = {} }) {
  return (
    <div className="absolute left-0 right-0 bottom-0 z-30"
      style={{
        height: 'calc(64px + env(safe-area-inset-bottom))',
        paddingBottom: 'env(safe-area-inset-bottom)',
        background: dark ? 'rgba(20,20,22,0.8)' : 'rgba(255,255,255,0.8)',
        ...
        {TABS.map((t) => {
          ...
          return (
            <button key={t.id} onClick={() => onTab(t.id)}
              className="flex-1 flex flex-col items-center justify-center gap-1 transition-transform"
              style={{ transform: on ? 'scale(1.08)' : 'scale(1)' }}>
              <span className="relative">
                <i className={`ph-fill ${t.icon}`}
                   style={{ fontSize: 24, color: on ? accent : (dark ? '#888' : '#9ca3af'), opacity: on ? 1 : 0.6 }} />
                {dots[t.id] && (
                  <span className="absolute -top-0.5 -right-1 rounded-full"
                        style={{ width: 8, height: 8, background: '#EF4444', boxShadow: '0 0 0 2px ' + (dark ? '#141416' : '#fff') }} />
                )}
              </span>
...
// ---------- Progress bar ----------
function Progress({ value, track = '#e5e7eb', height = 8, gradient = 'linear-gradient(135deg,#FF00FF,#FF66CC)' }) {
  return (
...
// ---------- Confetti ----------
function Confetti({ run = true, count = 80 }) {
...
Object.assign(window, {
  Photo, Avatar, Button, Toggle, Pill, Badge, VerifiedTick,
  Skeleton, Sheet, Toast, TabBar, Progress, Confetti,
});
