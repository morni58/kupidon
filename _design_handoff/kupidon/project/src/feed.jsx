/* ============================================================
   CupidBot — Feed (premium swipe), Match, Paywall
   parallax · mesh+grain · gloss · drag auras · spring physics
   ============================================================ */
const { useState: fUseState, useRef: fUseRef, useEffect: fUseEffect } = React;

const INTENTS = [
  { label: 'Без обязательств', emoji: '🎲' },
  { label: 'Один вечер', emoji: '🌙' },
];

// elegant glowing stamp (not a rubber stamp)
function Stamp({ show, text, color, side }) {
  ...
  return (
    <div className="absolute z-30 px-4 py-2 rounded-2xl font-black text-[22px] tracking-wide whitespace-nowrap"
      style={{ ...pos, color: '#fff', background: hexA(color, 0.22), border: `2.5px solid ${color}`,
        backdropFilter: 'blur(4px)', boxShadow: `0 0 30px ${hexA(color, 0.7)}, inset 0 0 18px ${hexA(color, 0.4)}`,
        opacity: show, transition: 'opacity .08s' }}>
      {text}
    </div>
  );
}
...
function ProfileCard({ person, theme, front, drag = { x: 0, y: 0 }, photoIdx = 0, onTapPhoto, dragging }) {
  ...
  const ph = person.photos[photoIdx];
  const dx = drag.x, dy = drag.y;
  const mag = Math.min(1, Math.abs(dx) / 130);
  ...
  // parallax: photo drifts opposite, slightly
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" style={{ borderRadius: '2rem', boxShadow: theme === 'oligarch' ? '0 30px 70px -18px rgba(255,215,0,0.28)' : adult ? '0 30px 70px -18px rgba(255,45,45,0.3)' : '0 30px 70px -18px rgba(255,0,255,0.28)' }}>
      ...
      <div className="absolute inset-0" style={{ transform: `translate(${par.x}px,${par.y}px) scale(1.08)`, transition: dragging ? 'none' : 'transform .4s cubic-bezier(.22,1,.36,1)' }}>
        ...
        {/* depth blobs from the other photos */}
        {person.photos[1] && <div className="mesh-b absolute -bottom-10 -right-8 rounded-full" style={{ width: 220, height: 220, background: `radial-gradient(circle,${person.photos[1].from},transparent 70%)`, opacity: 0.5, filter: 'blur(10px)' }} />}
        {person.photos[2] && <div className="mesh-c absolute -top-12 -left-10 rounded-full" style={{ width: 200, height: 200, background: `radial-gradient(circle,${person.photos[2].from},transparent 72%)`, opacity: 0.4, filter: 'blur(12px)' }} />}
      ...
      {front && (
        <div className="absolute inset-0 pointer-events-none" style={{
          background: `linear-gradient(105deg, transparent 35%, rgba(255,255,255,0.18) ${glossX}%, transparent ${glossX + 12}%)`,
          opacity: dragging ? 0.9 : 0.45, transition: 'opacity .3s' }} />
      )}

      {/* story indicators */}
      <div className="absolute top-3 left-3 right-3 flex gap-1.5 z-20">
        {person.photos.map((_, i) => (
          <div key={i} className="flex-1 rounded-full transition-all" style={{ height: 3, background: i === photoIdx ? '#fff' : 'rgba(255,255,255,0.4)', boxShadow: i === photoIdx ? '0 0 8px rgba(255,255,255,0.8)' : 'none' }} />
        ))}
      </div>
      ...
      {person.verified && (
        <div className="absolute top-7 right-3 z-20"><Badge kind="glass"><i className="ph-fill ph-seal-check" style={{ color: '#60A5FA' }} /> Verified</Badge></div>
      )}
      ...
      {/* bottom info — floating frosted glass panel */}
      <div className="absolute inset-x-3 bottom-3 z-20 rounded-[1.6rem] px-4 pt-3.5 pb-4 overflow-hidden"
        style={{ background: adult ? 'rgba(30,4,6,0.4)' : 'rgba(18,14,26,0.34)', backdropFilter: 'blur(18px) saturate(1.2)', WebkitBackdropFilter: 'blur(18px) saturate(1.2)', border: '1px solid rgba(255,255,255,0.18)', boxShadow: '0 12px 30px -12px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.22)' }}>
        <div className="flex items-end justify-between gap-2">
          ...
          <span className="flex items-center gap-1 text-[12px] font-semibold mb-0.5 shrink-0" style={{ color: 'rgba(255,255,255,0.85)' }}><i className="ph-fill ph-map-pin" style={{ color: adult ? '#FF6B6B' : '#FF99DD' }} />{person.dist}</span>
        </div>
        ...
        <div className="mt-2.5 flex flex-wrap gap-1.5 items-center">
          ...
          {!adult && <Badge kind="common"><i className="ph-fill ph-check-circle" /> 3 общих</Badge>}
        </div>
...
function SwipeCard({ person, theme, onDecide, photoIdx, onTapPhoto, fling }) {
  const [drag, setDrag] = fUseState({ x: 0, y: 0 });
  const [dragging, setDragging] = fUseState(false);
  const [leaving, setLeaving] = fUseState(null);
  const start = fUseRef(null);

  fUseEffect(() => {
    if (fling && fling.id === person.id) doFling(fling.dir);
  ...
  const move = (e) => {
    ...
    setDrag({ x: cx - start.current.x, y: cy - start.current.y });
  };
  const up = () => {
    if (!start.current) return;
    start.current = null; setDragging(false);
    const { x, y } = drag;
    if (x > 110) doFling('like');
    else if (x < -110) doFling('nope');
    else if (y < -120 && Math.abs(x) < 100) doFling('super');
    else setDrag({ x: 0, y: 0 });
  };
  const doFling = (dir) => {
    const target = dir === 'like' ? { x: 640, y: 30 } : dir === 'nope' ? { x: -640, y: 30 } : { x: 0, y: -780 };
    setLeaving(target);
    setTimeout(() => onDecide(dir), 320);
  };

  const pos = leaving || drag;
  const rot = pos.x / 16;
  const settling = !leaving && !dragging && drag.x === 0 && drag.y === 0;
  return (
    <div
      className="absolute inset-0 touch-none cursor-grab active:cursor-grabbing"
      style={{ transform: `translate(${pos.x}px, ${pos.y}px) rotate(${rot}deg)`, transition: leaving ? 'transform .34s cubic-bezier(.4,0,.2,1)' : settling || !dragging ? 'transform .42s cubic-bezier(.34,1.4,.5,1)' : 'none' }}
...
function ActionBar({ theme, plan, superLeft, onAction }) {
  const adult = theme === 'adult';
  ...
  const Round = ({ size, bg, children, onClick, glow, badge, ring }) => (
    <button onClick={onClick} className="relative rounded-full flex items-center justify-center active:scale-90 transition shrink-0"
      ...
      {children}
      {badge != null && (
        <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full flex items-center justify-center text-[10px] font-black text-white" style={{ background: '#A855F7', boxShadow: '0 0 0 2px #fff' }}>{badge}</span>
      )}
    </button>
  );
  return (
    <div className="flex items-center justify-center gap-3.5">
      <div className="relative">
        ...
        {!plan.rewind && <span className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 text-[8px] font-bold text-white/70 whitespace-nowrap flex items-center gap-0.5"><i className="ph-fill ph-lock-simple" />Premium</span>}
      </div>
  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center px-8 anim-pop overflow-hidden">
      ...
      <div className="relative flex items-center justify-center mb-7" style={{ height: 124 }}>
        <div className="floaty" style={{ animationDelay: '0s' }}><Avatar data={{ from: '#FF00FF', to: '#FF66CC', emoji: '😍' }} size={108} ring /></div>
...
function Paywall({ open, onClose, onUpgrade }) {
  const [t, setT] = fUseState('04:32:10');
  fUseEffect(() => {
    if (!open) return;
    ...
    <Sheet open={open} onClose={onClose}>
      <div className="text-center mb-1">
        ...
        <div className="inline-flex items-center gap-1.5 mt-3 px-3 py-1.5 rounded-full bg-[#FAFAFC] border border-[#e5e7eb]">
          <i className="ph-fill ph-clock text-[#6b7280]" />
          <span className="text-[13px] font-bold text-[#0F0F13] tabular-nums">Обновятся через {t}</span>
        </div>
        <p className="text-[11px] text-[#9ca3af] mt-1">в 00:00 МСК</p>
      </div>
      <div className="space-y-3 mt-5">
        <Button onClick={() => onUpgrade('premium')}>💎 Premium — 200 свайпов/день</Button>
        <Button variant="gold" onClick={() => onUpgrade('kupidon')}>👑 Kupidon VIP — 500 + Олигарх</Button>
...
function Feed({ theme, plan, swipesLeft, setSwipesLeft, onMatch, onUpgrade, pushToast, dots, active, onTab, openTariffs }) {
  const adult = theme === 'adult';
  const [idx, setIdx] = fUseState(0);
  const [photoIdx, setPhotoIdx] = fUseState(0);
  const [superLeft, setSuperLeft] = fUseState(plan.superlikes);
  const [match, setMatch] = fUseState(null);
  const [paywall, setPaywall] = fUseState(false);
  const [onlyVerified, setOnlyVerified] = fUseState(false);
  const [bump, setBump] = fUseState(0);
  const [fling, setFling] = fUseState(null);

  fUseEffect(() => { setSuperLeft(plan.superlikes); }, [plan.id]);

  const deck = PEOPLE.filter((p) => !onlyVerified || p.verified);
  const person = deck[idx];
  const accent = adult ? '#FF3333' : theme === 'oligarch' ? '#FFD700' : '#FF00FF';
  const planAura = plan.id === 'premium' ? hexA('#FF00FF', 0.5) : plan.id === 'kupidon' ? hexA('#FFD700', 0.55) : null;

  const decide = (dir) => {
    if (dir === 'super') { setSuperLeft((s) => s - 1); }
    setSwipesLeft((s) => Math.max(0, s - 1));
    setPhotoIdx(0); setFling(null);
    if ((dir === 'like' || dir === 'super') && person && (person.id === 'alisa' || Math.random() < 0.5)) {
      setTimeout(() => setMatch(person), 120);
    }
    setIdx((i) => i + 1);
    setBump((b) => b + 1);
  };
  ...
  const onAction = (dir) => {
    if (dir === 'rewind') {
      if (!plan.rewind) { pushToast('🔒 Откат доступен с Premium'); return; }
      if (idx > 0) { setIdx((i) => i - 1); setPhotoIdx(0); pushToast('⏪ Свайп возвращён'); }
      return;
    }
    if (swipesLeft <= 0) { setPaywall(true); return; }
    if (dir === 'super' && superLeft <= 0) { pushToast('⭐ Суперлайки закончились'); return; }
    setFling({ id: person.id, dir, n: bump });
  };

  const tapPhoto = (d) => setPhotoIdx((p) => Math.max(0, Math.min(person.photos.length - 1, p + d)));
  const ended = idx >= deck.length;

  return (
    <div className="w-full h-full flex flex-col relative overflow-hidden">
      <MeshBG palette={THEME_MESH[theme] || VIBES.neon} />
      {theme === 'adult' && <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(80% 60% at 50% 40%, rgba(255,45,45,0.18), transparent 70%)', animation: 'glowPulse 4s ease-in-out infinite' }} />}
      {planAura && <div className="absolute inset-0 pointer-events-none z-[2]" style={{ boxShadow: `inset 0 0 70px ${planAura}`, mixBlendMode: theme === 'light' ? 'multiply' : 'screen' }} />}
      ...
      <div className="relative z-10 w-full h-full flex flex-col">
        ...
        <div className="safe-top screen-pad shrink-0">
          <div className="flex items-center justify-between pt-1 pb-2">
            ...
            <button onClick={() => setOnlyVerified((v) => !v)}
              className="inline-flex items-center gap-1 rounded-full font-bold transition" style={{ height: 30, padding: '0 11px', fontSize: 11.5,
                background: onlyVerified ? 'rgba(59,130,246,0.18)' : (theme === 'light' ? 'rgba(255,255,255,0.6)' : 'rgba(255,255,255,0.08)'), color: onlyVerified ? '#3B82F6' : (theme === 'light' ? '#6b7280' : '#bbb'),
                border: `1.5px solid ${onlyVerified ? '#3B82F6' : (theme === 'light' ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.12)')}`, backdropFilter: 'blur(8px)' }}>
              <i className="ph-fill ph-seal-check" /> Только Verified
            </button>
          </div>
          <div className="text-right -mt-1 mb-1">
            <span className="text-[12px] font-bold" style={{ color: theme === 'light' ? '#9ca3af' : 'rgba(255,255,255,0.6)' }}>{swipesLeft} свайпов</span>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 screen-pad relative" style={{ paddingBottom: 8 }}>
          {ended ? (
            <div className="h-full flex flex-col items-center justify-center text-center px-6">
              ...
              <div className="mt-6 w-full"><Button onClick={() => { setIdx(0); setBump((b) => b + 1); }} style={adult ? { background: 'linear-gradient(135deg,#FF3333,#FF00FF)' } : theme === 'oligarch' ? { background: 'linear-gradient(135deg,#FFE259,#FFA751)', color: '#0F0F13' } : {}}>Расширить поиск</Button></div>
            </div>
          ) : (
            <div className="relative w-full h-full" style={{ filter: paywall ? 'blur(8px)' : 'none', transition: 'filter .3s' }}>
              ...
              <SwipeCard key={person.id + '-' + bump} person={person} theme={theme} onDecide={decide} photoIdx={photoIdx} onTapPhoto={tapPhoto} fling={fling} />
            </div>
          )}
        </div>
        ...
        {!ended && (
          <div className="shrink-0 screen-pad" style={{ paddingBottom: 'calc(64px + env(safe-area-inset-bottom) + 12px)', paddingTop: 4 }}>
            <ActionBar theme={theme} plan={plan} superLeft={superLeft} onAction={onAction} />
          </div>
        )}
      </div>

      <Paywall open={paywall} onClose={() => setPaywall(false)} onUpgrade={(p) => { setPaywall(false); onUpgrade(p); }} />
      {match && <MatchModal person={match} theme={theme} onWrite={() => { onMatch(match); setMatch(null); }} onContinue={() => setMatch(null)} />}
